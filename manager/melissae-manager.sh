...
cmd_help() {
    ...
    echo -e "  ${CYAN}inspector${RESET}                    Enable/disable or reconfigure the AI threat analyst"
...
# Write an .env that keeps Inspector disabled (no compose profile active)
_disable_inspector_env() {
    umask 077
    {
        echo "# Melissae manager environment - generated by the installer"
        echo "# Inspector AI analyst is disabled. Run 'inspector' to enable it."
        echo "COMPOSE_PROFILES="
    } > "$ENV_FILE"
    chmod 600 "$ENV_FILE" 2>/dev/null || true
}

# Interactively configure the optional Inspector AI threat-analyst service.
_configure_inspector() {
    ...
    case "${enable_inspector,,}" in
        y|yes) ;;
        *)
            _disable_inspector_env
            info "Inspector disabled. You can enable it later with the 'inspector' command."
            return 0
            ;;
    esac
    ...
    {
        echo "# Melissae manager environment - generated by the installer"
        echo "# Contains AWS Bedrock credentials for Inspector. Keep this file secret."
        echo "COMPOSE_PROFILES=inspector"
    ...
    chmod 600 "$ENV_FILE" 2>/dev/null || true
    success "Inspector enabled. Credentials saved to .env (chmod 600)."
}

# Enable/disable or reconfigure Inspector after installation
cmd_inspector() {
    _configure_inspector
    if ! ensure_compose_cmd; then return 1; fi
    if grep -q "^COMPOSE_PROFILES=inspector" "$ENV_FILE" 2>/dev/null; then
        info "Building and starting Inspector..."
        if "${compose_cmd[@]}" up -d --build melissae_inspector; then
            success "Inspector is running"
        else
            error "Failed to start Inspector"
            return 1
        fi
    else
        "${compose_cmd[@]}" rm -sf melissae_inspector >/dev/null 2>&1
        info "Inspector is disabled"
    fi
}
