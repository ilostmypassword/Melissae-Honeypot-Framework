...
_disable_inspektor_env() {
    umask 077
    {
        echo "# Melissae manager environment - generated by the installer"
        echo "# Inspektor AI analyst is disabled. Run 'inspektor' to enable it."
        echo "COMPOSE_PROFILES="
    } > "$ENV_FILE"
    chmod 600 "$ENV_FILE" 2>/dev/null || true
}

_configure_inspektor() {
    echo
    info "Inspektor - AI Threat Analyst (optional)"
    echo -e "  ${DIM}Inspektor is a LangChain agent backed by AWS Bedrock. It powers the"
    echo -e "  Inspektor page on the dashboard: chat with it and generate on-demand"
    echo -e "  threat briefings (exportable to PDF). It requires AWS Bedrock"
    echo -e "  credentials with model access.${RESET}"
    echo
    local enable_inspektor
    read -r -p "$(echo -e "${YELLOW}[?]${RESET} Enable Inspektor AI analyst? [y/N] ")" enable_inspektor
    case "${enable_inspektor,,}" in
        y|yes) ;;
        *)
            _disable_inspektor_env
            info "Inspektor disabled. You can enable it later with the 'inspektor' command."
            return 0
            ;;
    esac

    local aws_key aws_secret aws_token aws_region aws_model
    while true; do
        read -r -p "AWS Access Key ID: " aws_key
        [ -n "$aws_key" ] && break
        warn "Access Key ID cannot be empty"
    done
    while true; do
        read -r -s -p "AWS Secret Access Key: " aws_secret; echo
        [ -n "$aws_secret" ] && break
        warn "Secret Access Key cannot be empty"
    done
    read -r -s -p "AWS Session Token (optional, press Enter to skip): " aws_token; echo
    read -r -p "AWS Region [us-east-1]: " aws_region
    aws_region=${aws_region:-us-east-1}
    read -r -p "Bedrock Model ID [anthropic.claude-3-5-sonnet-20240620-v1:0]: " aws_model
    aws_model=${aws_model:-anthropic.claude-3-5-sonnet-20240620-v1:0}

    umask 077
    {
        echo "# Melissae manager environment - generated by the installer"
        echo "# Contains AWS Bedrock credentials for Inspektor. Keep this file secret."
        echo "COMPOSE_PROFILES=inspektor"
        echo "AWS_ACCESS_KEY_ID=${aws_key}"
        echo "AWS_SECRET_ACCESS_KEY=${aws_secret}"
        echo "AWS_SESSION_TOKEN=${aws_token}"
        echo "AWS_REGION=${aws_region}"
        echo "BEDROCK_MODEL_ID=${aws_model}"
    } > "$ENV_FILE"
    chmod 600 "$ENV_FILE" 2>/dev/null || true
    success "Inspektor enabled. Credentials saved to .env (chmod 600)."
}

# Enable/disable or reconfigure Inspektor after installation
cmd_inspektor() {
    _configure_inspektor
    if ! ensure_compose_cmd; then return 1; fi
    if grep -q "^COMPOSE_PROFILES=inspektor" "$ENV_FILE" 2>/dev/null; then
        info "Building and starting Inspektor..."
        if "${compose_cmd[@]}" up -d --build melissae_inspektor; then
            success "Inspektor is running"
        else
            error "Failed to start Inspektor"
            return 1
        fi
    else
        "${compose_cmd[@]}" rm -sf melissae_inspektor >/dev/null 2>&1
        info "Inspektor is disabled"
    fi
}
