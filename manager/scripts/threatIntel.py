...
# Detect rapid event bursts within a time window
...
# Compute threat score (0-100) from log analysis
...
# Fetch cached geo data from MongoDB
...
# Sanitize a geo string value for storage
...
# Batch geolocate IPs via ip-api.com with caching
...
# Fetch recent logs from MongoDB for analysis
...
# Upsert threat entries into MongoDB
...
# Analyze logs and generate threat intelligence
def process_logs(logs: List[Dict]) -> None:
    ip_data: Dict[str, List[Dict]] = defaultdict(list)
    ip_agents: Dict[str, set] = defaultdict(set)
    for entry in logs:
    ...
    for ip, entries in ip_data.items():
        ...
        threat_doc = {
            ...
            "reasons": reasons,
            "agents": sorted(ip_agents.get(ip, set())),
        }
        if first_seen:
